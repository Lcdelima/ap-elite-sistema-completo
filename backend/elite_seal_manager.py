...
async def require_module_access(user: dict, module: str):
    ...
    if not subscription:
        raise HTTPException(status_code=403, detail="Assinatura não encontrada")
    ...
    if validade and datetime.fromisoformat(validade) < datetime.now(timezone.utc):
...
class QRCodeService:
    ...
    def generate_qr_code(seal_id: str, hash_sha256: str) -> str:
        ...
        verify_url = f"{backend_url}/verify/{hash_sha256}"
...
class ManifestService:
    ...
    def create_manifest(
        ...
        return {
            ...
            "verificacao_publica": {
                "url": f"{os.environ.get('REACT_APP_BACKEND_URL', 'http://localhost:8001')}/verify/{hashes['sha256']}",
    ...
    def generate_pdf(manifest: dict, seal_id: str) -> str:
        ...
        <body>
            ...
            <div class="section">
                ...
                ''' for event in manifest['cadeia_custodia']])}
            </div>
            ...
            ''' if manifest['blockchain']['tx_hash'] else ''}
...
async def create_elite_seal(
    ...
    hashes = {
        ...
        "sha512": evidence.get("sha512", hashlib.sha512(evidence.get("sha256", "").encode()).hexdigest()),
    ...
    lacration_event = {
        ...
        "operador": current_user.get("name", current_user.get("email")),
        "ip": "system",  # TODO: capturar IP real
    ...
    seal_record = {
        ...
        "data_emissao": datetime.now(timezone.utc).isoformat(),
        "emitido_por": current_user.get("name", current_user.get("email")),
